...
function fetchRows(props) {
    ...
    return axios.get(route('panel.traffic-source-categories.data'), {
        params: {
            page,
...
async function saveRow() {
    ...
    const request = form.value.id
        ? axios.put(route('panel.traffic-source-categories.update', form.value.id), payload)
        : axios.post(route('panel.traffic-source-categories.store'), payload)
...
async function deleteRow(id) {
    ...
    if (confirmDialog) {
        confirmed = await confirmDialog({
            title: 'Excluir categoria de origem',
            message: 'Esta ação é permanente. Deseja realmente remover?',
            okLabel: 'Remover',
            okColor: 'negative',
        })
    } else if (typeof window !== 'undefined') {
        confirmed = window.confirm('Excluir esta categoria de origem?')
    }
    ...
    loading.value = true
    try {
        await axios.delete(route('panel.traffic-source-categories.destroy', id))
...
<template>
    <Head title="Categorias de Origem de Tráfego" />
