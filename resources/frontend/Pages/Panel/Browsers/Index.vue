...
function fetchRows(props) {
    ...
    return axios.get(route('panel.browsers.data'), {
        params: {
            page,
...
async function saveRow() {
    ...
    const request = form.value.id
        ? axios.put(route('panel.browsers.update', form.value.id), payload)
        : axios.post(route('panel.browsers.store'), payload)
...
async function deleteRow(id) {
    ...
    if (confirmDialog) {
        confirmed = await confirmDialog({
            title: 'Excluir navegador',
            message: 'Esta ação é permanente. Deseja realmente remover?',
            okLabel: 'Remover',
            okColor: 'negative',
        })
    } else if (typeof window !== 'undefined') {
        confirmed = window.confirm('Excluir este navegador?')
    }
    ...
    loading.value = true
    try {
        await axios.delete(route('panel.browsers.destroy', id))
        $q.notify({ type: 'positive', message: 'Navegador removido.' })
        await fetchRows({ pagination: pagination.value })
    } catch (error) {
        $q.notify({ type: 'negative', message: 'Falha ao remover o navegador.' })
    } finally {
        loading.value = false
    }
}
...
<template>
    ...
    <q-dialog v-model="dialogVisible" persistent>
        <q-card class="tw-w-full tw-max-w-2xl">
            <q-card-section>
                <div class="text-h6">{{ isEditing ? 'Editar navegador' : 'Novo navegador' }}</div>
            </q-card-section>
            <q-separator />
